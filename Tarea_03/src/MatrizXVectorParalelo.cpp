...
            }
        }
        for (unsigned int k = row; k < column; k++) {
        if(k<column){
           x[k] = rand() % 10;
           cout << "\t\t  [" << x[k] << "]";
           cout<<endl;
...
            column, // Numero de columnas a compartir
...
            column, // Numero de elementos que se van a enviar y recibir
